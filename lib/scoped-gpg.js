...
function gpg(args, stdio, keyring, callback) {
  args = [ '--no-default-keyring', '--keyring', keyring, '--status-fd=3' ]
      .concat(args);
  const p = spawn(GPG, args, { stdio: stdio });

  const stdout = p.stdout ? buffer(p.stdout) : (cb) => cb(null, null);
  const stderr = p.stderr ? buffer(p.stderr) : (cb) => cb(null, null);
  const status = p.stdio[3] ? buffer(p.stdio[3]) : (cb) => cb(null, null);
...
function getTTY(callback) {
  ...
  // TODO(indutny): Windows support somehow...
  fs.open('/dev/tty', 'r+', (err, fd) => {
    if (err)
      return callback(err);

    const res = new tty.ReadStream(fd);
    callback(null, new TTYPair(res, res));
  })
}
